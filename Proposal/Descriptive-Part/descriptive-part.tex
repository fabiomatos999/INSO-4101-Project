...
\begin{itemize}
  ...
  \item \hlc{green}{A shoe in the system-to-be can only have 1 owner at a time.\\
  ...
  \item \hlc{green}{Money associated to a sale in the system-to-be can to point to point.\\
  ...
  \item \hlc{green}{The verification process on the system-to-be is only concerned with shoes listed on the system-to-be.\\
...
\begin{itemize}
  ...
  \item \hlc{green}{The system-to-be must update a listing when an item sold so it in an invoiced state instead of being an active listing.}
  \item \hlc{green}{There cannot a sale on the system-to-be if there is no entry of the product.\\
  ...
  \item \hlc{green}{The system-to-be's checkout system has to be synchronous in order to avoid the scenario where the same item is sold to 2 or more different customers.}
  \item \hlc{green}{When an entry in the system-to-be is in an invoiced state, it will no longer appear when searched.}
  \item \hlc{green}{The system's entries can only be modified by an administrator. A customer only has the ability to buy an entry.}
\end{itemize}
\subsection{Machine Requirements}
\begin{itemize}
\item \hlc{green}{The system must serve 70000 fetch requests simultaneously.}
\item \hlc{green}{When fully loaded average response time for payment processing must be below 500ms and must be below 5 seconds in 99.9\% of all payment attempts}
  \item \hlc{green}{The system must be read by a screen reader, including any navigation menus.}
  \item \hlc{green}{The system must store the hashes for the passwords instead of the plain text.}
  \item \hlc{green}{The system must use established and battle tested APIs to deal with financial transactions.}
\end{itemize}
...
\begin{itemize}
  \item \hlc{green}{People/Businesses that want to sell their limited edition shoes (Limited Edition Shoe Sellers).\\ (Sellers) are relevant since they want to sell their product but do not have the reputation to successfully be able to sell these high price products on their own storefronts. They are also intrinsic to the domain since they provide the product to be sold.}
  \item \hlc{green}{People who want to buy limited edition shoes. (Limited Edition Shoe Buyers)\\ This group is relevant since they are the customer base for these kinds of products and are intrinsic to the domain.}
\end{itemize}
\subsection{Personas}
\begin{enumerate}
...
\begin{itemize}
  ...
  \item \hlc{green}{Management and organization\\ In the domain rough sketch it made mention that products and money sent to the company by sellers are held in trust until independent verification has concluded.}
\end{itemize}
\subsection{Business Processes}
\begin{itemize}
  \item \hlc{green}{Buyers: Limited Edition Shoe Sellers: As briefly mentioned in the domain rough sketch, the idea for moderating the listing of shoe entries on the website would be done by the company reviewer. A seller has to first fill out paperwork explaining the shoe to be reviews and then once the paperwork is processed, the seller would ship out the product to the company for review. If the product passed the review process it will be listed. Else the product will be returned and the seller would have a strike on their account or even be banned from selling on the platform.}
        \item \hlc{green}{Limited Edition Shoe Buyers: After a listing has been placed on the SNKRS Trust website, a buyer can click into a listing to see pictures of the product where they can purchase the item. Then the customer's money is held in trust until the item is shipped out to the customer.}
\end{itemize}
\section{Petri Net}
\createfigure{../Descriptive-Part/Figures/petri-net.png}{Petri Net for a Sale to occur}
